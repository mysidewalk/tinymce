...
var toc = (function () {
  ...
  var $_afhb95sbjj6614ip = { create: create };

  var tocId = $_afhb95sbjj6614ip.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_4k7dmesajj6614im.getTocClass(editor);
    var headerTag = $_4k7dmesajj6614im.getTocHeader(editor);
    var selector = generateSelector($_4k7dmesajj6614im.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_4k7dmesajj6614im.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_4k7dmesajj6614im.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_6h4f96s6jj6614if.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_6h4f96s6jj6614if.updateToc(editor);
    });
  };
  var $_ejuafgs5jj6614ic = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_4k7dmesajj6614im.getTocClass(editor);
  ...
  var $_9fa2bgscjj6614ir = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_6h4f96s6jj6614if.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_4k7dmesajj6614im.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_86h1ppsdjj6614it = { register: register$1 };

  global.add('toc', function (editor) {
    $_ejuafgs5jj6614ic.register(editor);
    $_86h1ppsdjj6614it.register(editor);
    $_9fa2bgscjj6614ir.setup(editor);
  });
  function Plugin () {
  }
