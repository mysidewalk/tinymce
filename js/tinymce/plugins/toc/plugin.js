...
var toc = (function () {
  ...
  var $_9eyu70rkjgyjyaek = { create: create };

  var tocId = $_9eyu70rkjgyjyaek.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_5saiukrjjgyjyaeg.getTocClass(editor);
    var headerTag = $_5saiukrjjgyjyaeg.getTocHeader(editor);
    var selector = generateSelector($_5saiukrjjgyjyaeg.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_5saiukrjjgyjyaeg.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_5saiukrjjgyjyaeg.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_23ppikrfjgyjyae1.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_23ppikrfjgyjyae1.updateToc(editor);
    });
  };
  var $_7t6unwrejgyjyadv = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_5saiukrjjgyjyaeg.getTocClass(editor);
  ...
  var $_1utodqrljgyjyaeo = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_23ppikrfjgyjyae1.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_5saiukrjjgyjyaeg.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_20h8rbrmjgyjyaeu = { register: register$1 };

  global.add('toc', function (editor) {
    $_7t6unwrejgyjyadv.register(editor);
    $_20h8rbrmjgyjyaeu.register(editor);
    $_1utodqrljgyjyaeo.setup(editor);
  });
  function Plugin () {
  }
