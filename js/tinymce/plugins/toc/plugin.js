...
var toc = (function () {
  ...
  var $_fpmq97rpjiwaaieq = { create: create };

  var tocId = $_fpmq97rpjiwaaieq.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_7p5h2krojiwaaiep.getTocClass(editor);
    var headerTag = $_7p5h2krojiwaaiep.getTocHeader(editor);
    var selector = generateSelector($_7p5h2krojiwaaiep.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_7p5h2krojiwaaiep.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_7p5h2krojiwaaiep.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_11p0cjrkjiwaaiej.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_11p0cjrkjiwaaiej.updateToc(editor);
    });
  };
  var $_bq1y6zrjjiwaaieg = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_7p5h2krojiwaaiep.getTocClass(editor);
  ...
  var $_gd71m8rqjiwaaier = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_11p0cjrkjiwaaiej.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_7p5h2krojiwaaiep.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_cgbt0frrjiwaaiet = { register: register$1 };

  global.add('toc', function (editor) {
    $_bq1y6zrjjiwaaieg.register(editor);
    $_cgbt0frrjiwaaiet.register(editor);
    $_gd71m8rqjiwaaier.setup(editor);
  });
  function Plugin () {
  }
